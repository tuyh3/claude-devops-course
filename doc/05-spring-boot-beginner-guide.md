...
> 🤔 **常见疑问**：Spring Boot 有几百个注解，都需要背下来吗？
...
**第一层：必须熟练的核心注解（约 20 个）**

这些在日常开发中高频使用，通过**写代码自然记住**：
...
// 测试
@SpringBootTest, @MockBean, @Test
```

**第三层：遇到再学的（剩余几百个）**

用到具体功能时再查，比如：
- 缓存相关：`@Cacheable`, `@CacheEvict`
...
| 场景 | 常用注解 |
|------|---------|
| 写 Controller | `@RestController`, `@GetMapping`, `@PostMapping` |
| 写 Service | `@Service`, `@Autowired`, `@Transactional` |
| 接收参数 | `@RequestBody`, `@PathVariable`, `@RequestParam` |
...
| 方法 | 说明 |
|------|------|
| **多写代码** | 写 3-5 个 CRUD 功能，核心注解自然就记住了 |
| **理解原理** | 知道 `@Autowired` 是依赖注入，比背语法重要 |
| **善用 IDE** | IDEA 的代码补全和文档提示非常强大 |
| **建立速查表** | 把常用的整理成一页笔记，需要时快速查阅 |
